...
namespace Article
{
    public partial class imgtxt : System.Web.UI.Page
    {
        ...
        //        System.Windows.Forms.MessageBox.Show("please enter username and select image.");
        //    }
        //    else
        //    {
        ...
        //        cmd.CommandText = "insert into Table1 values('" + path.ToString() + "','" + TextBox1.Text + "',@status,@username,getdate())";
                ...
                if (TextBox2.Text.Equals(loggedInUser, StringComparison.OrdinalIgnoreCase))
                {
                    if (IsUsernameExists(loggedInUser))
                    {
                        ...
                        if (Array.IndexOf(allowedFileExtensions, fileExtension.ToLower()) != -1)
                        {
                            ...
                            SqlCommand cmd = con.CreateCommand();
                            cmd.CommandType = CommandType.Text;
                            cmd.CommandText = "insert into Table1 values(@imgpath,@title,@status,@username,getdate())";
                        ...
                        {
                            System.Windows.Forms.MessageBox.Show("Invalid file format. Please upload only (.jpg, .jpeg, .png, .gif) image files.");
                        }
                    }
                    else
                    {
        ...
        private bool IsUsernameExists(string username)
        {
            ...
            using (SqlConnection regCon = new SqlConnection(@"Data source=SAHIL\SQLEXPRESS; initial catalog=userreg; integrated Security=true"))
            {
                regCon.Open();
